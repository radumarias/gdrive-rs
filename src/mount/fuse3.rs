...
use crate::fs::model::{CreateFileAttr, FileAttr, FileType, FsError, FsResult, SetFileAttr};
use crate::mount::{MountHandleInner, MountPoint};
use crate::{mount, GID, UID};
...
pub struct DirectoryEntryIterator(crate::fs::model::DirectoryEntryIterator, u64);
...
pub struct DirectoryEntryPlusIterator(crate::fs::model::DirectoryEntryPlusIterator, u64);
...
impl Fuse3 {
    pub async fn new(direct_io: bool, suid_support: bool) -> FsResult<Self> {
        Ok(Self {
            fs: crate::remote::wrapper::RemoteFilesystemImpl::new(direct_io, suid_support).await?,
            direct_io,
            suid_support,
        })
    }

    fn get_fs(&self) -> Arc<dyn crate::fs::Filesystem> {
        self.fs.clone()
    }
    ...
    async fn create_nod(
        ...
        let (fh, attr) = self
            .get_fs()
            .create(parent, name.to_str().unwrap(), attr, read, write)
            .await
            .map_err(|err| {
                error!(err = %err);
                match err {
...
impl Filesystem for Fuse3 {
    ...
    async fn lookup(&self, req: Request, parent: u64, name: &OsStr) -> Result<ReplyEntry> {
        ...
        let attr = match self
            .get_fs()
            .find_by_name(parent, name.to_str().unwrap())
            .await
        {
            Ok(Some(attr)) => attr,
            Err(err) => {
    ...
    async fn mkdir(
        ...
        let (_, attr) = self
            .get_fs()
            .create(parent, name.to_str().unwrap(), attr, false, false)
            .await
            .map_err(|err| {
                error!(err = %err);
                Errno::from(ENOENT)
    ...
    async fn rename(
        ...
        if new_parent_attr.perm & libc::S_ISVTX as u16 != 0 {
            if let Ok(Some(new_attrs)) = self
                .get_fs()
                .find_by_name(new_parent, new_name.to_str().unwrap())
                .await
            {
...
async fn mount_fuse(
    ...
    Ok(Session::new(mount_options)
        .mount_with_unprivileged(Fuse3::new(direct_io, suid_support).await?, mount_path)
        .await?)
}
